...
class SymfonyRequestTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $request = new Reo_Classic_Request();

        $request->initialize(array('foo' => 'bar'));
        $this->assertEquals('bar', $request->query['foo'], '->initialize() takes an array of query parameters as its first argument');

        $request->initialize(array(), array('foo' => 'bar'));
        $this->assertEquals('bar', $request->request['foo'], '->initialize() takes an array of request parameters as its second argument');

        $request->initialize(array(), array(), array('foo' => 'bar'));
        $this->assertEquals('bar', $request->attributes['foo'], '->initialize() takes an array of attributes as its third argument');

        $request->initialize(array(), array(), array(), array(), array(), array('HTTP_FOO' => 'bar'));
        $this->assertEquals('bar', $request->getHeaders('FOO'), '->initialize() takes an array of HTTP headers as its sixth argument');
    }
    ...
    {
        $request = Reo_Classic_Request::create('http://user_test:password_test@test.com/');
    ...
    {
        $request = Reo_Classic_Request::create('http://test.com/foo?bar=baz');
        ...
        $request = Reo_Classic_Request::create('http://test.com/foo', 'GET', array('bar' => 'baz'));
        $this->assertEquals('http://test.com/foo?bar=baz', $request->getUri());
        $this->assertEquals('/foo', $request->getPathInfo());
        $this->assertEquals('bar=baz', $request->getQueryString());
        $this->assertEquals(80, $request->getPort());
        ...
        $request = Reo_Classic_Request::create('http://test.com/foo?bar=foo', 'GET', array('bar' => 'baz'));
        $this->assertEquals('http://test.com/foo?bar=baz', $request->getUri());
        $this->assertEquals('/foo', $request->getPathInfo());
        $this->assertEquals('bar=baz', $request->getQueryString());
        $this->assertEquals(80, $request->getPort());
        ...
        $request = Reo_Classic_Request::create('https://test.com/foo?bar=baz');
        ...
        $request = Reo_Classic_Request::create('test.com:90/foo');
        ...
        $request = Reo_Classic_Request::create('https://test.com:90/foo');
        ...
        $request = Reo_Classic_Request::create('https://127.0.0.1:90/foo');
        ...
        $request = Reo_Classic_Request::create('https://[::1]:90/foo');
        ...
        $request = Reo_Classic_Request::create('https://[::1]/foo');
        ...
        $request = Reo_Classic_Request::create('http://example.com/jsonrpc', 'POST', array(), array(), array(), array(), $json);
        $this->assertEquals($json, $request->getContent());
        $this->assertFalse($request->isSecure());

        $request = Reo_Classic_Request::create('http://test.com');
        ...
        $request = Reo_Classic_Request::create('http://test.com?test=1');
        ...
        $request = Reo_Classic_Request::create('http://test.com:90/?test=1');
        ...
        $request = Reo_Classic_Request::create('http://username:password@test.com');
        $this->assertEquals('http://test.com/', $request->getUri());
        $this->assertEquals('/', $request->getPathInfo());
        $this->assertEquals('', $request->getQueryString());
        $this->assertEquals(80, $request->getPort());
        ...
        $request = Reo_Classic_Request::create('http://username@test.com');
        $this->assertEquals('http://test.com/', $request->getUri());
        $this->assertEquals('/', $request->getPathInfo());
        $this->assertEquals('', $request->getQueryString());
        $this->assertEquals(80, $request->getPort());
        ...
        $request = Reo_Classic_Request::create('http://test.com/?foo');
        $this->assertEquals('/?foo', $request->getRequestUri());
        $this->assertEquals(array('foo' => ''), $request->query);
    }
    ...
    public function testCreateCheckPrecedence()
    {
        ...
        $request = Reo_Classic_Request::create('/', 'DELETE', array(), array(), array(), array(
        ...
        $request = Reo_Classic_Request::create('http://thomas:pokemon@example.net:8080/?foo=bar', 'GET', array(), array(), array(), array(
            'HTTP_HOST' => 'example.com',
            'HTTPS' => 'on',
            'SERVER_PORT' => 443,
        ));
    ...
    {
        $request = new Reo_Classic_Request(array('foo' => 'bar'), array('foo' => 'bar'), array('foo' => 'bar'), array(), array(), array('HTTP_FOO' => 'bar'));
    ...
    {
        $request = new Reo_Classic_Request(array(), array(), array('_format' => 'json'));
        $dup = $request->duplicate();
    ...
    public function testGetUriForPath()
    {
        ...
        $request = Reo_Classic_Request::create('http://test.com:90/foo?bar=baz');
        ...
        $request = Reo_Classic_Request::create('https://test.com:90/foo?bar=baz');
    ...
    public function testGetQueryString($query, $expectedQuery, $msg)
    {
        ...
        $request->server['REQUEST_URI'] = 'example.com/?' . $query;
    ...
    {
        $request = Reo_Classic_Request::create('http://example.com', 'GET', array(), array(), array(), array(
            'HTTP_X_FORWARDED_PROTO' => 'https',
            'HTTP_X_FORWARDED_PORT' => '443',
        ));
        $request->setTrustedProxies(array());//localhost is set by default
        $port = $request->getPort();
        $this->assertEquals(80, $port, 'Without trusted proxies FORWARDED_PROTO and FORWARDED_PORT are ignored.');

        Reo_Classic_Request::setTrustedProxies(array('1.1.1.1'));
        //@note an extra check can be employed to verify the remote addr as to match the proxy, if there is only one layer
        $request = Reo_Classic_Request::create('http://example.com', 'GET', array(), array(), array(), array(
            'HTTP_X_FORWARDED_PROTO' => 'https',
        ...
        Reo_Classic_Request::setTrustedProxies(array());
    }
...
*/
    ...
    public function testGetClientIp($expected, $remoteAddr, $httpForwardedFor, $trustedProxies)
    {
        ...
        //@note filtering proxies was removed since the functionality exists
        $this->assertEquals($expected[0], $request->getClientIp());

        Reo_Classic_Request::setTrustedProxies(array());
        Reo_Classic_Request::disableHostCheck();
    }
    ...
    public function testCreateFromGlobals($method)
    {
        ...
        $request = Reo_Classic_Request::createFromGlobals();
        $this->assertEquals('bar1', $request->query['foo1'], '::fromGlobals() uses values from $_GET');
        $this->assertEquals('bar2', $request->request['foo2'], '::fromGlobals() uses values from $_POST');
        $this->assertEquals('bar3', $request->cookies['foo3'], '::fromGlobals() uses values from $_COOKIE');
        $this->assertEquals(array('bar4'), $request->files['foo4'], '::fromGlobals() uses values from $_FILES');
        ...
        $this->assertEquals('mycontent', isset($request->request['content']) ? $request->request['content'] : null);

        unset($_SERVER['REQUEST_METHOD'], $_SERVER['CONTENT_TYPE']);

        Reo_Classic_Request::createFromGlobals();
        Reo_Classic_Request::enableHttpMethodParameterOverride();
        ...
        $this->assertEquals('bar6', $request->request['foo6']);
    ...
    public function testIsXmlHttpRequest()
    {
        ...
        unset($request->headers['X-Requested-With']);
        $this->assertFalse($request->isXmlHttpRequest());
    }
    ...
    {
        $request = Reo_Classic_Request::create($uri, 'GET', array(), array(), array(), $server);
    ...
    private function getRequestInstanceForClientIpTests($remoteAddr, $httpForwardedFor, $trustedProxies)
    {
        ...
        if ($trustedProxies) {
            Reo_Classic_Request::setTrustedProxies($trustedProxies);
            Reo_Classic_Request::enableHostCheck();
        }
    ...
    {
        $request = Reo_Classic_Request::create('http://example.com/');
        $request->server['REMOTE_ADDR'] = '3.3.3.3';
        $headers = $request->getHeaders();//they're lazy loaded
        $request->headers['X_FORWARDED_FOR']  = '1.1.1.1, 2.2.2.2';
        $request->headers['X_FORWARDED_HOST'] = 'foo.example.com, real.example.com:8080';
        $request->headers['X_FORWARDED_PROTO'] = 'https';
        $request->headers['X_FORWARDED_PORT'] = 443;
        $request->headers['X_MY_FOR'] = '3.3.3.3, 4.4.4.4';
        $request->headers['X_MY_HOST'] = 'my.example.com';
        $request->headers['X_MY_PROTO'] = 'http';
        $request->headers['X_MY_PORT'] = 81;
        ...
        //@note in the real world a request doesn't change, so re-initalize to clear cached data from last request
        $request->initialize(array(), array(), array(), array(), array(), $server = array('REMOTE_ADDR' => '3.3.3.3'));
        $request->headers = $headers;//reapply headers
        Reo_Classic_Request::setTrustedProxies(array());
        //$this->assertEquals('3.3.3.3', $request->getClientIp());
        $this->assertEquals('example.com', $request->getHost());
        $this->assertEquals(80, $request->getPort());
        ...
        $request->initialize(array(), array(), array(), array(), array(), $server);
        $request->headers = $headers;//reapply headers
        Reo_Classic_Request::setTrustedProxies(array('3.3.3.3', '2.2.2.2'));
        ...
        $request->initialize(array(), array(), array(), array(), array(), $server);
        $request->headers = $headers;//reapply headers
        $request->headers['X_FORWARDED_PROTO'] = 'ssl';
        $this->assertTrue($request->isSecure());

        $request->initialize(array(), array(), array(), array(), array(), $server);
        $request->headers = $headers;//reapply headers
        $request->headers['X_FORWARDED_PROTO'] = 'https, http';
        $this->assertTrue($request->isSecure());

        // custom header names
        Reo_Classic_Request::setTrustedHeaderName(Reo_Classic_Request::HEADER_CLIENT_IP, 'X_MY_FOR');
        Reo_Classic_Request::setTrustedHeaderName(Reo_Classic_Request::HEADER_CLIENT_HOST, 'X_MY_HOST');
        Reo_Classic_Request::setTrustedHeaderName(Reo_Classic_Request::HEADER_CLIENT_PORT, 'X_MY_PORT');
        Reo_Classic_Request::setTrustedHeaderName(Reo_Classic_Request::HEADER_CLIENT_PROTO, 'X_MY_PROTO');
        $request->initialize(array(), array(), array(), array(), array(), $server);
        $request->headers = $headers;//reapply headers
        ...
        Reo_Classic_Request::setTrustedHeaderName(Reo_Classic_Request::HEADER_CLIENT_IP, null);
        Reo_Classic_Request::setTrustedHeaderName(Reo_Classic_Request::HEADER_CLIENT_HOST, null);
        Reo_Classic_Request::setTrustedHeaderName(Reo_Classic_Request::HEADER_CLIENT_PORT, null);
        Reo_Classic_Request::setTrustedHeaderName(Reo_Classic_Request::HEADER_CLIENT_PROTO, null);
        $request->initialize(array(), array(), array(), array(), array(), $server);
        $request->headers = $headers;//reapply headers
        $this->assertEquals('3.3.3.3', $request->getClientIp());
        $this->assertEquals('example.com', $request->getHost());
        ...
        // reset
        Reo_Classic_Request::setTrustedProxies(array());
        Reo_Classic_Request::setTrustedHeaderName(Reo_Classic_Request::HEADER_CLIENT_IP, 'X_FORWARDED_FOR');
        Reo_Classic_Request::setTrustedHeaderName(Reo_Classic_Request::HEADER_CLIENT_HOST, 'X_FORWARDED_HOST');
        Reo_Classic_Request::setTrustedHeaderName(Reo_Classic_Request::HEADER_CLIENT_PORT, 'X_FORWARDED_PORT');
        Reo_Classic_Request::setTrustedHeaderName(Reo_Classic_Request::HEADER_CLIENT_PROTO, 'X_FORWARDED_PROTO');
    }
    ...
    {
        Reo_Classic_Request::create('http://example.com/');
        Reo_Classic_Request::setTrustedHeaderName('bogus name', 'X_MY_FOR');
    }
    ...
    {
        Reo_Classic_Request::create('http://example.com/');
        Reo_Classic_Request::getTrustedHeaderName('bogus name');
    }
    ...
    public function testIISRequestUri($headers, $server, $expectedRequestUri)
    {
        ...
        $subRequest = Reo_Classic_Request::create($subRequestUri, 'GET', array(), array(), array(), $request->server);
    ...
    {
        // create a request
        $request = Reo_Classic_Request::create('/');
        ...
        Reo_Classic_Request::setTrustedHosts(array('.*\.?trusted.com$'));
        Reo_Classic_Request::enableHostCheck();

        // untrusted host
        $request->clearParamCache()->headers['HOST'] = 'evil.com';
        try {
            $request->getHost();
        ...
        // trusted hosts
        $request->clearParamCache()->headers['HOST'] = 'trusted.com';
        $this->assertEquals('trusted.com', $request->getHost());
        $this->assertEquals(80, $request->getPort());

        $request->clearParamCache()->server['HTTPS'] = true;
        $request->headers['HOST'] = 'trusted.com';
        $this->assertEquals('trusted.com', $request->getHost());
        $this->assertEquals(443, $request->getPort());
        $request->server['HTTPS'] = false;

        $request->clearParamCache()->headers['HOST'] = 'trusted.com:8000';
        $this->assertEquals('trusted.com', $request->getHost());
        $this->assertEquals(8000, $request->getPort());

        $request->clearParamCache()->headers['HOST'] = 'subdomain.trusted.com';
        $this->assertEquals('subdomain.trusted.com', $request->getHost());

        // reset request for following tests
        Reo_Classic_Request::setTrustedHosts(array());
        Reo_Classic_Request::disableHostCheck();
    }
    ...
    {
        Reo_Classic_Request::setTrustedProxies(array());//@note lose localhost
        $request = Reo_Classic_Request::create('/');
        $request->headers['HOST'] = $host;
        ...
        if ($isValid) {
            ...
            /*try{
                ...
                var_dump($e->getMessage(), $host);exit;
            }
            var_dump($host);exit;*/
...
class RequestClassicContentProxy extends Reo_Classic_Request
{
...
class NewRequestClassic extends Reo_Classic_Request
{
